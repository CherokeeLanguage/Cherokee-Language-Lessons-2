...
prev_already: pathlib.Path = pathlib.Path("cll1-ch21-final-written-already.txt")
...
def subject_objects() -> list[SubjectObject]:
    ...
    so_list.append(SubjectObject("ᎫᎫ", "3s", {"i", "neutral"}))
    so_list.append(SubjectObject("ᎫᎫ", "3p", {"i", "neutral"}))
    ...
    so_list.append(SubjectObject("ᎹᏫᎠ", "3s", {"a"}, True))
    so_list.append(SubjectObject("ᏆᏆ", "3s", {"a"}, True))
    so_list.append(SubjectObject("ᏆᏘᏏ", "3s", {"a"}, True))
    so_list.append(SubjectObject("ᏈᎭᏗ", "3s", {"a"}, True))
    so_list.append(SubjectObject("ᏙᏉᏘ", "3s", {"a"}, True))
    so_list.append(SubjectObject("ᏤᎻ", "3s", {"a"}, True))
    so_list.append(SubjectObject("ᏥᏂᏫ", "3s", {"a"}, True))
    so_list.append(SubjectObject("ᏦᏎᏫ", "3s", {"a"}, True))
    so_list.append(SubjectObject("ᏫᎵᎭᎹ", "3s", {"a"}, True))
...
def adjectives_list() -> list[Adjective]:
    ...
    a_list.append(Adjective("ᏐᎢ", "3s", {"i"}))
    a_list.append(Adjective("ᏐᎢ", "3s", {"a"}))
    a_list.append(Adjective("ᏗᏐᎢ", "3p", {"i"}))

    a_list.append(Adjective("ᎤᏁᎩᎸᏗ", "3s", {"i"}))
    a_list.append(Adjective("ᎤᏁᎩᎸᏗ", "3s", {"a"}))
    a_list.append(Adjective("ᏧᏁᎩᎸᏗ", "3p", {"i"}))
    a_list.append(Adjective("ᎤᏂᏁᎩᎸᏗ", "3p", {"a"}))

    a_list.append(Adjective("ᎤᏬᏚᎯ", "3s", {"i"}))
    a_list.append(Adjective("ᎤᏬᏚᎯ", "3s", {"a"}))
    a_list.append(Adjective("ᏧᏬᏚᎯ", "3p", {"i"}))
    a_list.append(Adjective("ᏧᏃᏚᎯ", "3p", {"a"}))
    ...
    a_list.append(Adjective("ᏗᎩᎦᎨ ᏧᏍᎪᎸ", "3p", {"i"}))
    a_list.append(Adjective("ᎠᏂᎩᎦᎨ ᎤᏂᏍᎪᎸ", "3p", {"a"}))
    a_list.append(Adjective("ᏗᎬᎿᎨ ᏧᏍᎪᎸ", "3p", {"i"}))
    a_list.append(Adjective("ᎠᏂᎬᎿᎨ ᎤᏂᏍᎪᎸ", "3p", {"a"}))
    a_list.append(Adjective("ᏗᏌᎪᏂᎨ ᏧᏍᎪᎸ", "3p", {"i"}))
    a_list.append(Adjective("ᎠᏂᏌᎪᏂᎨ ᎤᏂᏍᎪᎸ", "3p", {"a"}))
    a_list.append(Adjective("ᏗᏓᎶᏂᎨ ᏧᏍᎪᎸ", "3p", {"i"}))
...
def sentence_templates() -> list[str]:
    ...
    st.append("ᎥᏝ {adjective} {object} ᎦᏘᏲᎢ ᏱᎨᏎᎢ")
    st.append("{adjective} {object} ᎦᏘᏲᎢ ᎨᏎᏍᏗ")
...
less_used: dict[str, int] = dict()


def less_used_words(already_text: str) -> bool:
    """
    Check to see that at least 3/4 of the words are less used ones.
    :param already_text: str
    :return: bool
    """
    global less_used
    if not less_used:
        return True
    word_count:int = already_text.count(" ")+1
    less_count: int = 0
    count_median: int = median(less_used.values())
    for word in already_text.split():
        word = word.strip()
        cnt: int
        if word in less_used:
            cnt = less_used[word]
        else:
            cnt = 0
        if cnt < count_median:
...
def main() -> None:
        ...
        if verb.obj.startswith("3"):
            for _ in range(9):
                ...
                if verb_object.subj != verb.obj:
                    ...
                    continue
                if not verb.classes.issubset(verb_object.classes):
                    verb_object = SubjectObject()
                    continue
    ...
    with already_file.open("w") as w:
        for already_item in already:
            ...
            w.write("\n")
    counts_file = already_file.with_stem(already_file.stem + "-counts").with_suffix(".txt")
    with counts_file.open("w") as w:
        words: list[str] = [*less_used]
        words.sort()
        for word in words:
            w.write(f"{word}: {less_used[word]}\n")
